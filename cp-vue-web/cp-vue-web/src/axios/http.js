...
// let global = 'http://192.168.1.162/api/v1'; // 测试
//设置默认请求头
...
axios.interceptors.response.use(config => {
        // 返回请求正确的结果
        if (config.data.code === -1 || config.data.code === 990002) {
...
const install = (Vue) => {
    ...
    */
    // 注册邀请码状态接口 
    Vue.prototype.$inviteSwitch = (params, success) => get('/user/invite/switch', params, success);
    // 电费预估接口
    ...
    // 用户手机登录
    Vue.prototype.$phoneLogin = (params, success) => post('/user/phone/login', params, success);
    // 用户手机登录认证
